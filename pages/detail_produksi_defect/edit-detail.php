...
$sql = "SELECT * FROM detail_produksi_defect WHERE id = '$_POST[id]'";
...
<form id="edit-detail" enctype="multipart/form-data">
    <input type="hidden" name="id" value="<?= $row['id'] ?>">
    <input type="hidden" name="hasil_produksi_id" value="<?= $row['hasil_produksi_id'] ?>">
    <div class="row">
        <div class="col-lg-6">
            <div class="mb-3">
                ...
                <select name="produk_id" id="produk_id" class="form-control" required>
                    ...
                    <?php while($p = mysqli_fetch_array($produk_query)): ?>
                        <option value="<?= $p['id'] ?>" <?= $p['id'] == $row['produk_id'] ? 'selected' : '' ?>><?= $p['nama_produk'] ?> (<?= $p['kode_produk'] ?>)</option>
                    <?php endwhile; ?>
                </select>
            </div>
            <div class="mb-3">
                ...
                <select name="kategori_defect_id" id="kategori_defect_id" class="form-control" required>
                    ...
                    <?php while($k = mysqli_fetch_array($kategori_query)): ?>
                        <option value="<?= $k['id'] ?>" <?= $k['id'] == $row['kategori_defect_id'] ? 'selected' : '' ?>><?= $k['kategori_defect'] ?></option>
                    <?php endwhile; ?>
                </select>
            </div>
        </div>
        <div class="col-lg-6">
            ...
            <div class="mb-3">
                ...
                <textarea id="keterangan" name="keterangan" class="form-control" placeholder="Keterangan"><?= $row['keterangan'] ?></textarea>
            </div>
        </div>
    </div>
    <div class="row">
...
<script>
    $("#edit-detail").submit(function (e) {
        e.preventDefault();
        var formData = new FormData(this);
        $.ajax({
            type: 'POST',
            url: 'pages/detail_produksi_defect/proses.php?aksi=edit',
            data: formData,
            cache: false,
            contentType: false,
            processData: false,
